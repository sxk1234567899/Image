...
#include "appconfig.h"
VerifyDB::VerifyDB(QString userName,QObject *parent)
	: QObject(parent)
{
	connectStatus = false;
	dbName = "verify_" + userName;
	infoDataTableName = "artifact_info";
	imageDataTableName = "artifact_image";
	rawImageDataTableName = "artifact_raw_image";

	dbDir = AppConfig::instance()->appConf().verifyDBDir;
	QDir dir(dbDir);
	if (!dir.exists(dbDir))
	{
		dir.mkdir(dbDir);
...
bool VerifyDB::openDB()
{
	...
	if (!tableList.contains(rawImageDataTableName))
	{
		createRawImageDataTable();
	}
	
	return true;
}
void VerifyDB::closeDB()
{
	connectStatus = false;
...
bool VerifyDB::addInfoData(QHash<QString, QString> infoData)
{
	if (infoData.size() >= 11)
	{
...
bool VerifyDB::addImageData(QHash<QString, QString> infoData)
{
	if (infoData.size() >= 6)
	{
...
bool VerifyDB::addRawImageData(QHash<QString, QString> infoData)
{
	if (infoData.size() >= 6)
	{
		return sqliteDriver->addData(rawImageDataTableName, infoData);
	}
	else
	{
		qDebug() << "RegDB::addRawImageData" << "error" << "infoData size < 6";
		return false;
	}
}
bool VerifyDB::updateInfoData(QHash<QString, QString> infoData, QString number)
{
	QString sqlWhere = "where number = '" + number + "'";
	return sqliteDriver->updateData(infoDataTableName, infoData, sqlWhere);
}
bool VerifyDB::updateImageData(QHash<QString, QString> infoData, QString number, QString name)
{
	QString sqlWhere = "where number = '" + number + "' and name = '" + name + "'";
	return sqliteDriver->updateData(imageDataTableName, infoData, sqlWhere);
}
bool VerifyDB::updateRawImageData(QHash<QString, QString> infoData, QString number, QString name)
{
	QString sqlWhere = "where number = '" + number + "' and name = '" + name + "'";
	return sqliteDriver->updateData(rawImageDataTableName, infoData, sqlWhere);

}
bool VerifyDB::deleteInfoData(QString number)
{
	QString sqlWhere = "where number = '" + number + "'";
	if (sqliteDriver->delData(infoDataTableName, sqlWhere))
	{
		sqliteDriver->delData(rawImageDataTableName, sqlWhere);
		return sqliteDriver->delData(imageDataTableName, sqlWhere);
	}
	return false;
}
bool VerifyDB::deleteImageData(QString number, QString name)
{
    QString sqlWhere = "where number = '" + number + "' and name = '" + name + "'";
    return sqliteDriver->delData(imageDataTableName, sqlWhere);
}
bool VerifyDB::deleteRawImageData(QString number, QString name)
{
    QString sqlWhere = "where number = '" + number + "' and name = '" + name + "'";
    return sqliteDriver->delData(rawImageDataTableName, sqlWhere);
}
bool VerifyDB::getInfoData(QList<QHash<QString, QVariant>>& infoData)
{
	QString sqlWhere = "where is_deleted = 0";
    return sqliteDriver->getData(infoDataTableName, &infoData, sqlWhere);
}
bool VerifyDB::getImageData(QList<QHash<QString, QVariant>>& imageData, QString number)
{
    QString sqlWhere = "where is_deleted = 0 and number = '" + number + "'";
    return sqliteDriver->getData(imageDataTableName, &imageData, sqlWhere);
}
bool VerifyDB::getRawImageData(QList<QHash<QString, QVariant>>& imageData, QString number)
{
	QString sqlWhere = "where is_deleted = 0 and number = '" + number + "'";
    return sqliteDriver->getData(rawImageDataTableName, &imageData, sqlWhere);
}
void VerifyDB::creatInfoDataTable()
{
	if (connectStatus)
	{
		QList<DBTable>data;
        ...
        data.append(DBTable{ "verify_no","INTEGER NOT NULL" });					//验证步骤
		...
		data.append(DBTable{ "reg_agency","TEXT DEFAULT NULL" });				//登记机构			
		data.append(DBTable{ "verify_status","INTEGER NOT NULL" });				//验证状态
        data.append(DBTable{ "verify_result","TEXT DEFAULT NULL" });			//验证结果
		data.append(DBTable{ "create_time","TEXT NOT NULL" });					//创建时间
		data.append(DBTable{ "update_time","TEXT DEFAULT NULL" });				//更新时间
		data.append(DBTable{ "verify_time","TEXT DEFAULT NULL" });				//验证时间
		data.append(DBTable{ "is_deleted","INTEGER NOT NULL" });				//删除状态
		sqliteDriver->creatTable(dbName, infoDataTableName, data);
	}
}
void VerifyDB::creatImageDataTable()
{
	if (connectStatus)
	{
		QList<DBTable>data;
		data.append(DBTable{ "id" ,"INTEGER PRIMARY KEY AUTOINCREMENT" });
        data.append(DBTable{ "image_id","INTEGER DEFAULT NULL" });
...
void VerifyDB::createRawImageDataTable()
{
	if (connectStatus)
	{
		QList<DBTable>data;
		...
		data.append(DBTable{ "update_time","TEXT DEFAULT NULL" });
		data.append(DBTable{ "is_deleted","INTEGER NOT NULL" });
		sqliteDriver->creatTable(dbName, rawImageDataTableName, data);
	}
}