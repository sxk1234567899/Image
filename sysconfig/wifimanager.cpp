...
WiFiManager* WiFiManager::self = nullptr;
...
bool WiFiManager::openWLAN(std::map<std::string, int>& wlanMap)
{
...
bool WiFiManager::connectWLAN(const std::string wlanName)
{
...
bool WiFiManager::passwordToConnectWLAN(const std::string wlanName, const std::string password)
{
...
bool WiFiManager::setProfile(const std::string wlanName, const std::string password)
{
	std::string strProfile = mWiFiProfileXml;
	stringReplace(strProfile, mWiFiName, wlanName);
	stringReplace(strProfile, mWiFiPassword, password);
...
wchar_t* WiFiManager::stringToWideChar(const std::string pKey)
{
...
void WiFiManager::stringReplace(std::string &inputoutopt, const std::string oldStr, const std::string newStr)
{
...
std::string WiFiManager::getError()const
{
...
bool WiFiManager::refreshWLAN(std::map<std::string, int>& wlanMap)
{
	wlanMap.clear();
	...
	if (numberOfItems > 0)
	{
		for (DWORD i = 0; i < numberOfItems; i++)
		{
			...
			if (wlanAN.dot11Ssid.uSSIDLength != 0)
			{
				...
				int wifiQuality = std::ceil((double)wlanAN.wlanSignalQuality/20);//返回信号的强度
...
bool WiFiManager::queryInterface(std::string &connectedWifi, int& signalQuality)
{
	if (!pIfInfo)
		return false;
	connectedWifi.clear();
